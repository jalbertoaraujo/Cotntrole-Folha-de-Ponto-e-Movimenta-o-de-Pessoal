...
public class RelatorioGeralFaltantesPorPeriodoeContrato2Dao {
	
	private Connection connection;
	
	private String contratos;

	public RelatorioGeralFaltantesPorPeriodoeContrato2Dao(){
	...
	public void RelatorioGeralFaltantesDao(){
		...
		String sql = "SELECT contrato, Count(contrato) AS faltantes FROM colaborador where periodo = '"+ TelaAuditoria.cbperiodoauditoria.getSelectedItem() +"' and  verificado = '---' and tipo = 'MANUAL' and contrato in (" + contratoFormatado + ") GROUP BY contrato order by contrato";	
	...
	public void RelatorioGeralDevolvidosDao(){
...
//		contratoFormatado = contratoFormatado.substring(0, contratoFormatado.length()-1);
		...
		String sql = "SELECT contrato, Count(contrato) AS faltantes FROM colaborador where periodo = '"+ TelaAuditoria.cbperiodoauditoria.getSelectedItem() +"' and  verificado = 'DEVOLVIDO' GROUP BY contrato order by contrato";	
		...
		try {
			...
			JRResultSetDataSource jrrs = new JRResultSetDataSource(rs);
			JasperPrint jp = JasperFillManager.fillReport("RelIreport/RelatorioGerencialTotalDevolvidosContratos.jasper", new HashMap(), jrrs);
			JasperViewer.viewReport(jp,false);       
