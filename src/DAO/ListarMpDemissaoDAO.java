...
public class ListarMpDemissaoDAO {

	private Connection connection;

	public ListarMpDemissaoDAO() {
	...
	public void listar() {
		...
		DefaultTableModel tab = (DefaultTableModel) MovimentacaoDePessoal.tabelademissao.getModel();

		String sql = "SELECT * FROM demissao order by id_demissao";
		...
		for (Demissao temp : dados) {
			Object[] linha = { temp.getId_demissao(), temp.getNome(), temp.getMatricula(), temp.getDatadeinicio(), temp.getContrato(), temp.getTipodemissao(), temp.getUltimodia(), temp.getDesconto(),
					temp.getPcd(), temp.getAuditoria(), temp.getMotivodesligamento(), temp.getNecessidade(), temp.getMp(), temp.getObsmp(), temp.getValidar()};
			tab.addRow(linha);
		}
	}
	
	public void listarbuscademissao() {

		List<Demissao> dados = new ArrayList<Demissao>();

		DefaultTableModel tab = (DefaultTableModel) MovimentacaoDePessoal.tabelademissao.getModel();

		String sql = "SELECT * FROM demissao WHERE matricula = '"+ MovimentacaoDePessoal.tfbuscademissao.getText() +"' OR contrato LIKE '%"+ MovimentacaoDePessoal.tfbuscademissao.getText() +"%' OR nome LIKE '"+ MovimentacaoDePessoal.tfbuscademissao.getText() +"%'";
