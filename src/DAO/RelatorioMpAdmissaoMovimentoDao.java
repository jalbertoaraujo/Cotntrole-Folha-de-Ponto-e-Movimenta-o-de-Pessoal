...
public class RelatorioMpAdmissaoMovimentoDao {
	
	private Connection connection;

	public RelatorioMpAdmissaoMovimentoDao(){
	...
	public void RelatorioMpAdmissaoDao(){
		...
		int indiceLinh = MovimentacaoDePessoal.tabelaadmissao.getSelectedRow();
		
		String sql =  "select admissao.id_admissao,admissao.periodo,funcionario,data,nome,setor,autorizacao,indicacao,responsavel,dataautorizacao,contrato,datainicio,idade,sexo,escolaridade,funcao,salario,adsalario,percentual,horario,escala,intervalo,jornada,vt,obrigatorio,vr,outrosben,bairro,expfun,contratacao,admissaoprazo,prazo,motivo,situacaomp,validar,obs,info,datavalidar from admissao inner join validaradmissao on admissao.id_admissao = validaradmissao.id_admissaovalidar where admissao.id_admissao = '"+ MovimentacaoDePessoal.tabelaadmissao.getValueAt(indiceLinh, 0) +"'";
		
	//	String sql = "SELECT * FROM admissao WHERE id_admissao = '"+ MovimentacaoDePessoal.tabelaadmissao.getValueAt(indiceLinh, 0) +"' ";
