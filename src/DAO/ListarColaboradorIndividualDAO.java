...
public class ListarColaboradorIndividualDAO {

	private Connection connection;

	public ListarColaboradorIndividualDAO() {
	...
	public void listar() {
		...
		String sql = "SELECT * FROM colaborador WHERE nome LIKE '" + Ponto.tfbuscacolaborador.getText()
				+ "%' OR matricula = '" + Ponto.tfbuscacolaborador.getText() + "' AND periodo = '"
		...
		for (Colaborador temp : dados) {
			Object[] linha = { temp.getId_colaborador(), temp.getMatricula(), temp.getNome(), temp.getFuncao(), temp.getAdmissao(), temp.getPeriodo(), temp.getTipo(), temp.getVerificado() };
			tab.addRow(linha);
		}
		if (dados.isEmpty()) {
